...
export function getPlayerSportMmr(player: { footballMmr?: number; cricketMmr?: number; tournamentFootballMmr?: number; tournamentCricketMmr?: number; mmr?: number }, sportType: string, isTournament: boolean = false): number {
  if (isTournament) {
    if (sportType.includes('CRICKET')) return player.tournamentCricketMmr ?? player.mmr ?? 1000;
