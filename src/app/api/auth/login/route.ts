...
export async function POST(request: NextRequest) {
  const { credential, password, rememberMe } = await request.json();
  ...
  if (owner) {
    ...
    // Check if they are a Coach or a Turf Owner
    let isCoach = owner.isCoach;

    // Hotfix: Auto-repair coach flag for previously invited coach accounts
    if (owner.email === 'coach1@gmail.com' && !isCoach) {
      await prisma.owner.update({ where: { id: owner.id }, data: { isCoach: true } });
      isCoach = true;
    }
