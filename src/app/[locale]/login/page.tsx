  ...
  return (
    <div className="relative flex-1 flex flex-col items-center justify-center min-h-screen bg-background overflow-hidden">
      ...
      <div className="w-full max-w-sm px-4 relative z-10">
        ...
        {/* Card */}
        <form onSubmit={handleSubmit} className="glass-panel rounded-3xl p-6 flex flex-col gap-5 shadow-[0_20px_60px_rgba(0,0,0,0.6)]">
          <AuthInput
            label={t('emailLabel')}
            type="text"
            ...
            rightElement={
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="text-neutral-500 hover:text-[var(--foreground)] transition-colors"
                >
                  {showPassword ? <EyeOff size={14} /> : <Eye size={14} />}
                </button>
                <Link href="/forgot-password" className="text-[11px] font-bold text-accent hover:underline">
                  {t('forgotPassword')}
                </Link>
              </div>
            }
          ...
          <label className="flex items-center gap-3 cursor-pointer select-none group -mt-1">
            <div
              ...
              className={`relative w-10 h-5 rounded-full transition-all duration-300 shrink-0 ${
                rememberMe ? 'bg-accent shadow-[0_0_10px_rgba(0,255,65,0.4)]' : 'bg-neutral-700'
              }`}
            >
            ...
            <span className={`text-xs font-bold transition-colors ${rememberMe ? 'text-accent' : 'text-neutral-500 group-hover:text-neutral-300'}`}>
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-1 w-full bg-accent text-black font-black py-3.5 rounded-xl hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-[0_4px_20px_rgba(0,255,0,0.2)] text-sm tracking-wide disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {loading ? (
