...
export interface ShopFrontInitialData {
  slides: Slide[];
  settings: Settings;
  categories: Category[];
  products: Product[];
...
interface ShopFrontClientProps {
  initialData: ShopFrontInitialData;
}

export default function ShopFrontClient({ initialData }: ShopFrontClientProps) {
      ...
      <div ref={scrollRef} onScroll={snapScroll} className="flex w-full h-full overflow-x-auto snap-x snap-mandatory hide-scrollbar">
        {slides.map((s) => (
          <div key={s.id} className="w-full h-full shrink-0 snap-center relative">
            <img src={s.imageUrl} alt={s.ctaText || 'BMT Shop promotion'} className="w-full h-full object-cover" />
