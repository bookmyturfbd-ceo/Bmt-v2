      ...
      <div className="relative z-10 w-full max-w-md mx-auto rounded-t-3xl border-t border-x border-white/10 overflow-hidden flex flex-col"
        ...
        <div className="px-5 pt-4 pb-28 flex flex-col gap-4 overflow-y-auto" style={{ maxHeight: 'min(85dvh, 85vh)' }}>
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-black">{t('confirmBooking')}</h2>
          ...
          <div className="flex flex-col gap-3 glass-panel border border-white/5 rounded-2xl p-4">
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-[9px] font-black uppercase tracking-widest text-[var(--muted)]">{t('turf')}</p>
                <p className="text-sm font-black truncate">{turfName}</p>
                {ground && <p className="text-[10px] text-[var(--muted)]">{t('ground')}: {ground.name}</p>}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-[var(--muted)]">{t('date')}</p>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-[var(--muted)]">{t('time')}</p>
          ...
          {availableSports.length > 1 && (
            <div className="flex flex-col gap-2 glass-panel border border-white/5 rounded-2xl p-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--muted)] flex items-center gap-1.5"><Tag size={12}/> {t('selectGameFormat')}</p>
          ...
          {!groupId && (
            <div className="flex flex-col gap-2">
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--muted)]">{t('couponCode')}</p>
              {couponApplied ? (
                <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl border border-accent/30 bg-accent/5">
                  <CheckCircle2 size={14} className="text-accent" />
                  <p className="text-sm font-black text-accent flex-1">{coupon.toUpperCase()} {t('applied')} −৳{couponDiscount.toLocaleString()}</p>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={applyCoupon} className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-xs font-black hover:bg-white/10 transition-colors">{t('apply')}</button>
                </div>
              )}
              {couponError && <p className="text-[10px] font-bold text-red-400 flex items-center gap-1"><AlertTriangle size={10} /> {t('invalidCoupon')}</p>}
            </div>
          )}
          ...
          {groupId && (
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-cyan-400">{t('groupSplitBreakdown')}</p>
              </div>
                    ...
                    return (
                      <div key={m.id} className={`flex items-center gap-3 px-3 py-2 rounded-xl border ${
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {hasInsufficient && (
                            <p className="text-[9px] text-red-400 font-bold">{t('insufficientWallet')}</p>
                          )}
                        </div>
                    ...
                    return (
                      <>
                        <div className={`flex items-center justify-between px-3 py-2 rounded-xl border mt-1 ${
                          ...
                          <span className="text-[10px] font-black text-neutral-400">{t('totalAllocated')}</span>
                        ...
                        {!isAllocationExact && (
                          <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl border border-red-500/30 bg-red-500/5 mt-1">
                            ...
                            <p className="text-xs font-bold text-red-400">
                              {t('allocationMismatch')} (৳{totalAllocated.toLocaleString()} / ৳{slot.price.toLocaleString()})
                            </p>
                          </div>
                        )}
                      </>
              ...
              {groupCulprits.length > 0 && (
                <div className="flex flex-col gap-1 px-3 py-2.5 rounded-xl border border-orange-500/30 bg-orange-500/5">
                  <p className="text-[10px] font-black text-orange-400 flex items-center gap-1"><AlertTriangle size={11} /> {t('lowWallet')}</p>
                  {groupCulprits.map((c: any) => (
                    <p key={c.playerId} className="text-[10px] text-orange-300 font-bold pl-4">
                      {c.name} {t('needMore')} ৳{c.shortfall.toLocaleString()}
                    </p>
                  ))}
                  <p className="text-[10px] text-neutral-500 font-bold pl-4 mt-0.5">{t('askTopUp')}</p>
                </div>
              )}
            </div>
          )}
          ...
          {!groupId && (
            <>
              <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-black">{t('walletBalance')}</p>
                  <p className={`text-[10px] font-bold ${!hasEnough ? 'text-red-400' : 'text-[var(--muted)]'}`}>
                    {walletBalance === null ? t('loading') : `৳${(walletBalance).toLocaleString()} ${t('available')}`}
                  </p>
                </div>
                {walletBalance !== null && (
                  <div className="text-right shrink-0">
                    <p className="text-[9px] text-[var(--muted)] font-bold uppercase tracking-wide">{t('afterBooking')}</p>
              ...
              {!hasEnough && walletBalance !== null && (
                <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl border border-red-500/30 bg-red-500/5">
                  ...
                  <p className="text-xs font-bold text-red-400">{t('insufficientWalletBalance')}</p>
                </div>
              )}
            </>
          )}
          ...
          <div className="flex flex-col gap-2 px-4 py-3 rounded-2xl bg-white/[0.03] border border-white/5">
            <div className="flex items-center justify-between text-sm">
              <span className="text-[var(--muted)] font-semibold">{t('slotPrice')}</span>
              {slotSaving > 0 ? (
            ...
            {slotSaving > 0 && (
              <div className="flex items-center justify-between text-sm">
                <span className="flex items-center gap-1.5 font-semibold" style={{ color: '#00ff41' }}>
                  <Tag size={11} />
                  {slotDiscountReason || t('slotDiscount')}
                </span>
            ...
            {!groupId && couponDiscount > 0 && (
              <div className="flex items-center justify-between text-sm">
                <span className="text-accent/80 font-semibold">{t('couponDiscount')}</span>
          ...
          {groupId ? (
            <button
              ...
              {booking ? t('booking') : (!selectedSport ? t('selectGameFormat') : t('confirmBooking'))}
            </button>
          ) : (
            <button
              onClick={handleBook}
              ...
              {booking ? t('booking') : (!selectedSport ? t('selectGameFormat') : (!hasEnough ? t('insufficientBalance') : `${t('bookFor')} ৳${finalPrice.toLocaleString()}`))}
            </button>
          )}
        </div>
      </div>
...
export default function TurfBookingClient({ turfId, turfName, area, cityName, slots, bookings, grounds, sports, groupId }: Props) {
  ...
  const handleConfirmClick = useCallback(() => {
    if (!isAuthed) {
      window.location.href = window.location.origin + '/' + locale + '/login?next=' + encodeURIComponent(window.location.pathname);
      return;
    }
    if (!selectedSlotId || !selectedDate) {
      alert(t('selectSlotAlert'));
      return;
    }
    setShowConfirm(true);
  }, [isAuthed, selectedSlotId, selectedDate, locale, t]);
        ...
        <div className="glass-light dark:glass rounded-3xl border border-white/10 shadow-[0_-4px_40px_rgba(0,0,0,0.8)] px-5 pt-3.5 pb-4 flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider">
              {selectedSlot ? t('selected') : t('startingFrom')}
            </span>
              ...
              {slotDiscountedPrice !== undefined && selectedSlotId ? (
                <>
                  ...
                  <div className="flex items-baseline gap-1.5">
                    ...
                    <span className="text-[11px] text-neutral-400 font-bold">/{locale === 'bn' ? 'স্লট' : 'slot'}</span>
                  </div>
                </>
              ) : (
          ...
          {isAuthed === null ? (
            ...
            <button onClick={handleConfirmClick} disabled={!selectedSlotId}
              ...
              {selectedSlotId ? t('confirmBooking') : t('selectSlot')}
            </button>
          ) : (
