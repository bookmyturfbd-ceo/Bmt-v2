...
import {
  Wallet, ChevronRight, ChevronLeft, Upload,
...
export default function WalletDashboard() {
  ...
  if (!dataLoaded && view === 'overview') {
    return (
      <div className="w-full max-w-md mx-auto rounded-3xl border border-white/10 flex flex-col items-center justify-center gap-4 py-32"
        style={{ background: 'linear-gradient(180deg, #0a1209 0%, #07080e 100%)' }}>
  ...
  return (
    <div
      className="w-full max-w-md mx-auto rounded-3xl border border-white/10 overflow-hidden flex flex-col shadow-2xl"
      style={{ minHeight: '65vh', background: 'linear-gradient(180deg, #0a1209 0%, #07080e 100%)' }}
    >
      {/* ═══ OVERVIEW VIEW ═══ */}
      {view === 'overview' && (
        <>
          {/* Balance header */}
          <div className="px-5 pt-6 pb-5 flex items-start justify-between shrink-0">
            <div>
          ...
          {/* Bento grid */}
          <div className="px-4 flex flex-col gap-3 flex-1 pb-8">
            {/* 2 bento boxes */}
            ...
            {!submitted && step === 'amount' && (
              <div className="flex flex-col gap-4">
                ...
                <input value={amount} onChange={e => setAmount(e.target.value.replace(/\D/g, ''))}
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-sm font-bold outline-none focus:border-[#00ff41]/40 placeholder:text-neutral-600 text-white animate-none" />
