  ...
  const handlePublish = async () => {
    ...
    try {
      const payload = {
        ...
        formatConfig: { ...formatConfig, sportVariant: form.sportVariant },
      };
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex gap-3 mb-3">
                  {['FOOTBALL', 'CRICKET'].map(s => (
                    <button
                      ...
                      onClick={() => {
                        set('sport', s);
                        set('sportVariant', s === 'FOOTBALL' ? 'FUTSAL_5' : 'CRICKET_7');
                      }}
                ...
                {/* Format Variant */}
                <label className={labelCls}>Format Variant</label>
                <select
                  value={form.sportVariant}
                  onChange={e => set('sportVariant', e.target.value)}
                  className={inputCls}
                >
                  {form.sport === 'FOOTBALL' ? (
                    <>
                      <option value="FUTSAL_5">5-a-side Futsal</option>
                      <option value="FUTSAL_6">6-a-side Football</option>
                      <option value="FUTSAL_7">7-a-side Football</option>
                      <option value="FOOTBALL_FULL">11-a-side Football</option>
                    </>
                  ) : (
                    <>
                      <option value="CRICKET_7">7-a-side Cricket</option>
                      <option value="CRICKET_FULL">11-a-side Cricket</option>
                    </>
                  )}
                </select>
              </div>
                ...
                <div className="p-5 space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-neutral-400 font-bold uppercase tracking-wider">{form.sportVariant.replace('_', ' ')} · {FORMATS.find(f => f.value === form.formatType)?.label}</p>
                    </div>
